...
export async function POST(req: Request) {
  try {
    ...
    let marketBlock = ''
    if (market_intel && market_intel.award_count !== undefined) {
 ...
 + Number(r.total).toLocaleString() + ')'
      ...
      const sizeLine =
        mi.median_award !== undefined
          ? 'Award size in this NAICS/state: 
...
Return your verdict as JSON only.`
 ...
 + Number(mi.min_award).toLocaleString() + ' to 
...
Return your verdict as JSON only.`
 ...
 + Number(mi.max_award).toLocaleString() + ' (median 
...
Return your verdict as JSON only.`
 ...
 + Number(mi.median_award).toLocaleString() + ').'
          : ''
      marketBlock =
