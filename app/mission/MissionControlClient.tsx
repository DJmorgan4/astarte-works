...
export function MissionControlClient() {
  ...
  return (
    <div style={{
      height: '100vh', background: S.void, color: S.text, fontFamily: S.mono,
      display: 'grid', gridTemplateRows: '44px 1fr', gridTemplateColumns: '1fr 380px',
      overflow: 'hidden',
    }}>
      <header style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <div style={{ display: 'flex', gap: 16, fontSize: 8, letterSpacing: '0.14em' }}>
            {['STRATUM','LOCUS','NEXUS'].map(s => <span key={s} style={{ color: S.amberDim }}>{s}</span>)}
          </div>
        </div>
      ...
      <main style={{ overflow: 'hidden', display: 'flex', flexDirection: 'column', padding: 16, gap: 12 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 8 }}>
          {[
            ...
            <div key={m.label} style={{ background: S.panel, border: `1px solid ${m.hot ? S.borderHot : S.border}`, padding: '10px 14px' }}>
        ...
        <div style={{ display: 'flex', borderBottom: `1px solid ${S.border}` }}>
          {(['alerts','sites'] as const).map(t => (
            <button key={t} onClick={() => setActiveTab(t)} style={{
              ...
              fontSize: 8, letterSpacing: '0.2em', fontFamily: S.mono,
        ...
        <div style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 2 }}>
          {activeTab === 'alerts' && alerts.map(a => {
            ...
            return (
              <div key={a.id} style={{
                ...
                display: 'grid', gridTemplateColumns: '44px 1fr 32px', gap: 10, alignItems: 'center',
              }}>
          ...
          {activeTab === 'sites' && sites.map(s => (
            <div key={s.id} style={{
              padding: '8px 12px', background: S.panel, border: `1px solid ${S.border}`,
              display: 'grid', gridTemplateColumns: '1fr 56px', gap: 10, alignItems: 'center',
            }}>
              <div>
          ...
          {activeTab === 'alerts' && alerts.length === 0 && (
            <div style={{ padding: 24, fontSize: 9, color: S.textDim, textAlign: 'center', letterSpacing: '0.14em' }}>NO ACTIVE ALERTS</div>
          )}
        </div>
      ...
      <aside style={{ borderLeft: `1px solid ${S.borderHot}`, background: S.panel, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <div style={{ padding: '10px 16px', borderTop: `1px solid ${S.borderHot}`, display: 'flex', gap: 8 }}>
          <input value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && send()}
            ...
            style={{ flex: 1, background: S.void, border: `1px solid ${S.border}`, color: S.text, fontSize: 11, padding: '8px 12px', fontFamily: S.mono, outline: 'none' }}
          />
